...
const canvasId = document.currentScript.getAttribute("canvasId");
...
async function init() {
  ...
  document.getElementById(canvasId).appendChild(app.view);

  const aliceTexture = await PIXI.Assets.load(assetPath("alice.png"));
  const bobTexture = await PIXI.Assets.load(assetPath("bob.png"));
...
function setUpAliceBob(texture, xPos, balance, person) {
  ...
  sprite.width = sprite.height = canvasWidth / 5;
...
function createReloadButton(texture) {
  ...
  reloadSprite.height = reloadSprite.width = canvasWidth / 8;
...
function createOverlay() {
  ...
  obj.drawRect(0, 0, canvasWidth, canvasHeight);
  obj.visible = false;
...
function createCoin(texture) {
  ...
  coinSprite.width = canvasWidth / 10;
...
function assetPath(assetName) {
  return `/animations/token-payments/images/${assetName}`;
}
